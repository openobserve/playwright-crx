...
// `noTestId` is ours: it forces a pass that ignores the test attribute entirely, so the
// ranked candidate list contains at least one selector that does not break when that
// attribute is renamed. Everything else here is upstream's.
type InternalOptions = GenerateSelectorOptions & { noText?: boolean, noCSSId?: boolean, noTestId?: boolean, isRecursive?: boolean };
