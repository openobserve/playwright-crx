...
// patch(playwright-crx): 1.58 made this component own all of its state and talk to the
// app through RecorderBackend/RecorderFrontend, which crx adopts as-is. These two stay
// as props because they have no counterpart in that contract: the crx popup lets the
// author *edit* the generated code, so it needs the editor's change and cursor events,
// and both are React callbacks rather than a request/response the backend could answer.
...
export const Recorder: React.FC<RecorderProps> = ({
  ...
  // patch(playwright-crx): only steal focus into the locator editor when the pick came
  // from a real user gesture, so programmatic picks don't yank the caret mid-edit.
  ...
  React.useLayoutEffect(() => {
    const dispatcher: RecorderFrontend = {
      ...
      // patch(playwright-crx): replaces the whole log instead of merging into it. crx
      // replay rebuilds every entry on each step, so merging accumulates stale rows —
      // an earlier run's steps stay on screen underneath the current one.
      callLogsReplaced: ({ callLogs }) => {
        setLog(() => {
      ...
      elementPicked: ({ elementInfo, userGesture }) => {
        ...
        if (mode === 'inspecting' && selectedTab === 'aria') {
          ...
          // patch(playwright-crx): upstream returns to 'recording' from anything that is
          // not plain 'inspecting', which silently promotes a *standby* pick into
          // recording. Decide from whether we were actually recording.
          const isRecording = ['recording', 'assertingText', 'assertingVisibility', 'assertingValue', 'assertingSnapshot'].includes(mode);
          backend.setMode({ mode: isRecording ? 'recording' : 'standby' }).catch(() => { });
        }
      },
    };
  ...
  return <div className='recorder'>
    ...
    <SplitView
      sidebarSize={200}
      main={<CodeMirrorWrapper text={source.text} highlighter={source.language} highlight={source.highlight} revealLine={source.revealLine} readOnly={source.id !== 'playwright-test'} onChange={onEditedCode} onCursorActivity={onCursorActivity} lineNumbers={true} />}
      sidebar={<TabbedPane
        ...
        tabs={[
          {
            id: 'locator',
            title: 'Locator',
            render: () => <CodeMirrorWrapper text={locator} placeholder='Type locator to inspect' highlighter={source.language} focusOnChange={selectorFocusOnChange} onChange={onEditorChange} wrapLines={true} />
          },
          {
            id: 'log',
            title: 'Log',
