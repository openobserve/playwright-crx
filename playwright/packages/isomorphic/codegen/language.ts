...
export function generateCode(actions: actions.ActionInContext[], languageGenerator: LanguageGenerator, options: LanguageGeneratorOptions) {
  ...
  // patch(playwright-crx): actions are generated BEFORE the header, though still joined
  // after it. crx keeps upstream's pre-1.62 behaviour of emitting openPage/closePage for
  // secondary pages in test mode, which means the generated test needs `context` in its
  // fixtures — and whether it does is only known once the generator has seen every page.
  // 1.62 sidestepped this by dropping those actions from test output entirely; we do not,
  // because a recorded popup that silently vanishes from the code is worse than an extra
  // fixture.
  const footer = languageGenerator.generateFooter(options.saveStorage);
  const actionTexts = actions.map(a => languageGenerator.generateAction(a, options)).filter(Boolean);
  const header = languageGenerator.generateHeader(options, languageGenerator.usesContext?.());
