...
export class JavaScriptLanguageGenerator implements LanguageGenerator {
  ...
  generateAction(actionInContext: actions.ActionInContext, options: LanguageGeneratorOptions): string {
    ...
    // patch(playwright-crx): skip these only for the PRIMARY page. Upstream 1.62 skips
    // them for every page in test mode, which drops a recorded popup out of the generated
    // code entirely; crx keeps emitting `context.newPage()` for secondary pages.
    if (this._isTest && pageAlias === 'page' && (action.name === 'openPage' || action.name === 'closePage'))
      return '';
